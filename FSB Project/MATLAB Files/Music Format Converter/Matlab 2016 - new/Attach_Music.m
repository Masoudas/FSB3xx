...
for i = 1:150
    ...
    if (exist(Music_Path_String, 'file') == 2)  % If this file exists,
        ...
        if ((audio_info.SampleRate ~= 32e3) || (audio_info.BitsPerSample ~= 8) || ~strcmp(audio_info.CompressionMethod,'Uncompressed'))
...
for i = 1:120
    ...
    if (exist(Music_Path_String, 'file') == 2)  % If this file exists,
        ...
        Music_vector = [Music_vector; Music_vector(end-zero_pad-1:end-zero_pad,1)];
    ...
    elseif (Initial_page < 8192 && Final_page >= 8192 && ~isempty(Music_vector)) % If a file has to be divided between flash one and two
...
if (Final_page < 8192)
    msgbox('One memory chip on FSB board is sufficient!')
elseif (Final_page >= 8192 && Final_page < 8300)
...
if (Final_page <= 16200)
    ...
    if (Final_page >= 8192) % If there is data to be recorded on the second memory.
