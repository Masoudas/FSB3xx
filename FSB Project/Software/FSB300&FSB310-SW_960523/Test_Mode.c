...
unsigned char Test_Input_Detection(struct ControlSignal *CTRL_Signal_Params, struct NumeratorSignal *NUM_Signal_Params)
{
     ...
     //// Main commands
     current_test_input = (int((*NUM_Signal_Params).current_input) << 8) + int((*CTRL_Signal_Params).current_input & CTRL_Test_DontC_bits);     // Define an integer number that contains both the numerator and control signals
