...
#define         START_UP_DELAY            210     // Denotes the start up delay of the board before reading the emergency signal
#define         EMR_DELAY                 110     // Denotes the delay before playing OVL message (OVL_DELAY*program_tick).
#define         OVL_DELAY                 210     // Denotes the delay before playing OVL message (OVL_DELAY*program_tick).
#define         DOB_DELAY                 310     // Denotes the delay before playing DOB message (DOB_DELAY*program_tick).
...
#define         MUSIC_AFTER_STOP          525     // The length of music played after declaring stop status (MUSIC_AFTER_STOP*program_tick).
...
#define         SERIAL_DEBOUNCE_COUNTER          20     // Debounce counter for the serial input.
...
#define         I2C_DATA_LENGTH                 3       // The number of bytes to be received via I2C.
...
#define         MIN_VOLUME_LEVEL                3       // Minimum defined volume.
#define         UPSAMPLE_FACTOR                 2       // The upsampling rate of the board, set at two.
#define         TOTAL_MEMORY_FILES              120     // All files that are loaded in the memory(es)    
...
//// Other globals
extern    unsigned char                buffer_data[264];         // Data read from flash at each call of Flash_Page_Read(). 
