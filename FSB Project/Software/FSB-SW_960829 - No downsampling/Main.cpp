...
*@ Version:	1.0
*@ Update:	Last:95/07/14	Previous:95/10/07	Created:95/10/07
...
// Other globals
extern          unsigned char                    buffer_data[528] = {0};
...
{  
     ...
     while(1)
     {
        ...
        if (Status_Register == READY)
        {
               ...
               Flash_Data_Read(&state_timer, current_album, &page_address[ALBUM1_FILE_NUMBER][0], &page_address[EXTRA_ALBUM1_FILE_NUMBER][0], &page_address[EXTRA_ALBUM2_FILE_NUMBER][0]);  // Play music, message or remain silent based on the current state.
...
{     
  ...
  switch (current_state)
  {
    case (EMR): // If it is emergency after POR (only time emergency state may occur).
      ...
      state_timer = page_address[6][1] - page_address[6][0];  // Play the entire music designated to emergency state.
    ...
    case (SILENCE_2): // Check to see if any input test signals are given; if not, the board enters silence mode.
      ...
      {       
        ...
        state_timer = page_address[test_input_number+6][1]-page_address[test_input_number+6][0];    
    ...
    case (SILENCE): // If current state is silence.
      if (CTRL_Signal_Params.CTRL_Signal == EMR_SIGNAL) // If the control signal is emergency.
      {
        ...
        state_timer = page_address[0][1] - page_address[0][0]; // Play the entire EMR message,
      ...
      else if ((CTRL_Signal_Params.CTRL_Signal == OVL_SIGNAL) && (OVL_Delay_Counter > OVL_DELAY)) // If overload signal is received and its delay has overflown.
      {
        ...
        state_timer = page_address[3][1] - page_address[3][0]; // Play the entire OVL message,
      ...
      else if ((CTRL_Signal_Params.CTRL_Signal == DOB_SIGNAL) && (DOB_Delay_Counter > DOB_DELAY))
      {
        ...
        state_timer = page_address[2][1] - page_address[2][0];   // Play the entire DOB message,
    ...
    case (DEBOUNCE_NUM_SIGNAL):
      ...
      {
        next_state = DING; 
        state_timer = page_address[4][1] - page_address[4][0]; // Timer is set according to ding length
      ...
      else if (NUM_Signal_Params.current_stop < NUM_Signal_Params.previous_stop)  // If stop number has lowered.
      {
        ...
        state_timer = page_address[1][1] - page_address[1][0]; // Timer is set to ding-dong length.
    ...
    case (DING):
      ...
      state_timer = page_address[NUM_Signal_Params.stop_track_index][1] - page_address[NUM_Signal_Params.stop_track_index][0]; // Stop status message for FSB310 corresponds to stop_track_index.
...
void Parameter_Initial_Setup(void)
{
     ...
     check_synchronous_loop_status = SYNCHRONOUS_LOOP_UNCHECKED;
...
{
    for (unsigned int i = 0 ; i < 528 ; i++)    // Set zero data in the music buffer.
    {
       buffer_data[i] = 0;
    } 
}
...
__interrupt void out_now(void)
{
     ...
     if (buffer_counter > 527)  // If the entire (current) page has been read, restart the counter.
     {
       buffer_counter = 0;
       check_synchronous_loop_status = SYNCHRONOUS_LOOP_UNCHECKED;
     }
     ...
     if (buffer_counter > 510 && check_synchronous_loop_status == SYNCHRONOUS_LOOP_UNCHECKED)
     {
       check_synchronous_loop_status = SYNCHRONOUS_LOOP_CHECKED;  
