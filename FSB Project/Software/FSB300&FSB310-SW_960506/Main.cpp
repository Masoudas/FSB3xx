...
{  
     ...
     while(1)
     {
        ...
        {
          I2C_output_flag = I2C_Protocol(&I2C_data[0], I2C_DATA_LENGTH, I2C_SR_MODE, I2C_SLAVE_ADDRESS, I2C_BITRATE, I2C_FREQUENCY_PRESCALAR, state_timer);
          if (I2C_output_flag == I2C_PROTOCOL_FINISHED)
          {
            Serial_CTRL_Signal_Detection(&CTRL_Signal_Params, start_up_flag, I2C_data); // Determine the current control signal.
            stop_track_index = Serial_NUM_Signal_Detection(&NUM_Signal_Params, I2C_data); // Determine the current numerator signal.
          }         
        }
        ...
        if (Status_Register == READY)
        {
               ...
               half_page_counter = Flash_Data_Read(half_page_counter, &state_timer, current_album, &page_address[ALBUM1_FILE_NUMBER][1], &page_address[EXTRA_ALBUM1_FILE_NUMBER][1], &page_address[EXTRA_ALBUM2_FILE_NUMBER][1]);  // Play music, message or remain silent based on the current state.
               timer_update();  // Update system timer
               VOL_Signal_Detection(&VOL_Signal_Params); // Determine the current volume signal signal.
               ...
               if (Board_Type == FSB300)
               {
                 ...
                 stop_track_index = Parallel_NUM_Signal_Detection(&NUM_Signal_Params);    
               }
               
               // Check for new state if state_timer has elapsed.
               if (state_timer < 1)  
               {     
                 current_state = Change_State(current_state);  // Determine the current state based on the input signal.
...
{     
  ...
  switch (current_state)
  {
    ...
    case (SILENCE_2): // Check to see if any input test signals are given; if not, the board enters silence mode.
      ...
      else if (VOL_Signal_Params.keys_current_state == VOL_UP_INPUT)
      {
        next_state = SILENCE_2;
        state_timer = 0; 
      }
    ...
    case (MUS):
      ...
      {
        next_state = MUS;
        state_timer = 4;        // Check for input signal every 4 program ticks while playing music.
    ...
    case (DING):
      ...
      state_timer = UPSAMPLE_FACTOR*(page_address[stop_track_index][1] - page_address[stop_track_index][0]); // Stop status message for FSB310 corresponds to stop_track_index.
      STOP_STATUS_current_page = page_address[stop_track_index][0];
    break;
    ...
    case (MUS_3):
      ...
      else if ((current_album == 1) && (MUS_current_page < page_address[ALBUM1_FILE_NUMBER][1])) // // If there is no album change signal and if the entire music of main album has not been played.
      {
         next_state = MUS_3;  // Keep on playing music.
         state_timer = 4;  // Play music of the current album till the end, unless a change album signal arrives.
      }
      else if ((current_album == 2) && (MUS_current_page < page_address[EXTRA_ALBUM1_FILE_NUMBER][1])) // Same for the first extra album
      {
         next_state = MUS_3; 
         state_timer = 4;  // Play music of the current album till the end, unless a change album signal arrives.
      }
      else if ((current_album == 3) && (MUS_current_page < page_address[EXTRA_ALBUM2_FILE_NUMBER][1])) 
      {
         next_state = MUS_3;   
         state_timer = 4;  // Play music of the current album till the end, unless a change album signal arrives.
      }
    ...
    case (ALBUM_CHANGE):
      // Increasing the album number
      current_album++;
      if (current_album > NUMBER_OF_ALBUMS)
      {
        current_album = 1;
      }
...
void Parameter_Initial_Setup(void)
{
     ...
     start_up_flag = 0;
     new_stop_arrival_flag = 0;
     I2C_output_flag = I2C_PROTOCOL_FINISHED;
...
bool Board_Type_Detection(void)
{
     ...
     // Main commands
     DDRB = 0x00;     // Temporarily define PB6 as input.
     PORTB = 0x20;    // Temporarilty enable the pull-up of this port.

     for (unsigned char i = 0; i < BOARD_TYPE_DEBOUNCE_NUMBER; i++)
     {
       vote += (PINB&0x20);
     }
     
     // Finalization
     if (vote < ((BOARD_TYPE_DEBOUNCE_NUMBER+1)/2))
     {
       return(FSB300);
     }
     else
...
void Page_Extraction(void)
{
     ...
     SPI_Interface(CON_ARRAY_READ); // Sending the opcode D2H to initialize continuous reading.     
     ...
     {
       // Initial page
       page_address[file_index][0] = SPI_Interface(Dont_Care_Byte) << 8; // First byte of initial page address is multiplied by 256!
       page_address[file_index][0] += SPI_Interface(Dont_Care_Byte); 
       
       // Final page
       page_address[file_index][1] = SPI_Interface(Dont_Care_Byte) << 8; // First byte of final page address is multiplied by 256!
