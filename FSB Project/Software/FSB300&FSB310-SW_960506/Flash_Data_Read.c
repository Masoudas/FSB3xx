...
unsigned char Flash_Data_Read(unsigned char half_page_counter, unsigned int *state_timer, char current_album, unsigned int *Album1, unsigned int *Album2, unsigned int *Album3)
{
     // Initialization
     ...
     switch (current_state)
     {
       ...
       case (MUS_2):
                 MUS_current_page += Flash_Page_Read(MUS_current_page,half_page_counter);  // Play the following pages of the (rather relaxing!) music for emergency state.
       break;
...
void MUS_Page_Reset(char current_album, unsigned int *Album1, unsigned int *Album2, unsigned int *Album3)
{
    // Initialization
  
  
    // Main commands
    if ((current_album == 1) && (MUS_current_page >= *(Album1+1))) // If the entire music of main album has been played.
    {
       MUS_current_page = *Album1;  // Restart from the top, else continue from current page.
    }
    else if ((current_album == 2) && (MUS_current_page >= *(Album2+1))) // Else if we are in the extra album 1 and we have exceeded its final page, restart from the top of this album
    {
       MUS_current_page = *Album2;  // Restart from the top of music track, else continue from current page.
    }
    else if ((current_album == 3) && (MUS_current_page >= *(Album3+1))) // Same for the second extra album
    {
...
unsigned char Flash_Page_Read(unsigned int page_number, unsigned char half_page_counter)
{
     ...
     {
       memory_number = 2;
       page_number = page_number - (FINAL_PAGE + 1);    // the page number for memory two is given an offset of 8100 pages so that it starts from zero as well.
     }
...
void Music_Deciphering(int index)
{
   //// Initialization     
